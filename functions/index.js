/* Daily Bake — Cloud Functions (Google Cloud / Firebase, region europe-west1)
 *
...
const SITE_URL = 'https://mahbaz-12e0c.web.app';
...
async function pushToTokens(tokens, { title, body, image, url, tag }) {
  ...
  for (let i = 0; i < tokens.length; i += 500) {
    ...
    const res = await admin.messaging().sendEachForMulticast({
      ...
      data: { url: url || '/index.html', tag: tag || 'dailybake' },
      webpush: {
        headers: { Urgency: 'high', TTL: '86400' },
        notification: { icon: '/assets/logo.png', badge: '/assets/logo.png', ...(image ? { image } : {}), tag: tag || 'dailybake', renotify: true, dir: 'rtl', lang: 'ar' },
