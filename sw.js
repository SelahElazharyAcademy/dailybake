...
const CACHE = 'dailybake-shell-v49';
...
self.addEventListener('push', (e) => {
  let data = {};
  try { data = e.data ? e.data.json() : {}; } catch (err) { data = { title: e.data ? e.data.text() : 'Daily Bake' }; }
  const title = data.title || 'Daily Bake';
  e.waitUntil(self.registration.showNotification(title, {
    body: data.body || '',
    icon: data.icon || '/assets/logo.png',
    badge: '/assets/logo.png',
    image: data.image || undefined,
    tag: data.tag || 'dailybake',
    renotify: true,
    dir: 'rtl',
    lang: 'ar',
    vibrate: [120, 60, 120],
