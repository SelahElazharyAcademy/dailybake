...
export async function renderFeatures(container) {
  ...
  container.innerHTML = `
    <div class="ex-eg-card ex-eg-narrow">
      ...
      ${fr('f-pwa', ICONS.grid, 'تثبيت الموقع كتطبيق', 'اقتراح تثبيت Daily Bake على شاشة العميل + يشتغل بدون إنترنت', features.pwaInstall)}
...
export async function renderTelegram(container) {
  ...
  $('#test-tg').addEventListener('click', async () => {
    ...
    try {
      const res = await fetch(`https://api.telegram.org/bot${token()}/sendMessage`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ chat_id: chatId, text: '✅ رسالة تجريبية من لوحة تحكم Daily Bake' }) });
