    ...
    if (upd.message && typeof upd.message.text === 'string' && upd.message.text.startsWith('/start')) {
      ...
      if (payload) {
        try {
          ...
          await tgCall('sendMessage', { chat_id: chatId, text: '🥐 تمام! إشعارات Daily Bake اتفعّلت على تليجرام.\nهنبعتلك كل تحديث لطلبك وكل منتج جديد أو خصم.' });
          handled++;
        } catch (e) { /* الاشتراك مش موجود */ }
      } else {
        await tgCall('sendMessage', { chat_id: chatId, text: `أهلاً بيك في Daily Bake 🥐\nرقم الشات بتاعك: ${chatId}` });
      }
      continue;
    }
