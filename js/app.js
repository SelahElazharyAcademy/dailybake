...
(function () {
  'use strict';
  let DATA = { name: 'Daily Bake', categories: [], currencyCode: 'EGP', fallbackProductImage: 'assets/logo.png?v=2' };
  ...
  function applyData(d) {
    ...
    document.title = `${DATA.name || 'Daily Bake'}`;
  ...
  function updateSearchMetadata() {
    const storeName = tPlain(DATA.name, DATA.name) || 'Daily Bake';
    ...
    const structuredData = {
      ...
      '@type': 'WebSite',
      name: storeName,
      url: 'https://mahbaz-12e0c.web.app/',
      description,
      hasPart: (DATA.categories || []).flatMap(category => (category.products || []).map(product => ({
        '@type': 'Product',
        name: tPlain(product.name, 'منتج'),
        image: product.image || DATA.logo || 'https://mahbaz-12e0c.web.app/assets/logo.png',
