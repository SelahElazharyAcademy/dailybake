...
## رفع الصور — "مفيش صلاحية للحفظ" كان حجم مش صلاحيات (2026-09-15)
قاعدة `assets/$id` بترفض أي نص > **900,000 حرف**، وفايربيز بيرد على رفض الـ **validate**
بنفس كود `PERMISSION_DENIED` بتاع رفض الصلاحية — فاللوحة كانت بتقول "مفيش صلاحية للحفظ"
والمشكلة إن الصورة كبيرة. (الدليل وقتها: أكبر أصل مخزَّن = 888,972 حرف، على حافة الحد.)
السبب الجذري في `compressImage`: مع `maxBytes = 0` كان بيطلع **PNG بلا فقد وبدون سقف**
لأي ملف PNG/WebP — وصور المنتجات PNG بمتوسط 2.3 ميجا.
- **`MAX_IMAGE_CHARS = 760000`** في `js/imageUtils.js` = الميزانية الافتراضية لكل ضغط.
- الشفافية اتحافظ عليها بـ **WebP** بدل PNG (`supportsWebp()` بيسأل الكانفس). PNG مايستجيبش
  لمعامل الجودة خالص، فلو المتصفح مابيعرفش يكتب WebP بنسطّح على أبيض ونطلع JPEG.
- `fitDataUrl()` بتعدّي أي data URL جاهز على نفس الميزانية — مسار **إزالة الخلفية**
  (كان بيطلّع PNG بالمقاس الكامل) بيعدّي عليها، و`publishImage` بتستعملها كحاجز أخير.
- رسائل الخطأ بقت "الحفظ اترفض — غالباً الصورة كبيرة أو مفيش صلاحية" بدل ما تجزم بالصلاحية.
- قياس فعلي: PNG 2400×2400 شفاف (3.5 ميجا) → WebP 120 ك.ب في ~240ms.

## صورة عبر رابط (2026-09-15)
الحقل كان موجود بس بلا أي تحقق. دلوقتي زر **تحقق** + فحص تلقائي بعد 600ms بيحمّل الرابط
فعلاً ويقول "شغال ✓ (العرض×الطول)" أو إنه مش بيفتح صورة، ودعم **دروب بوكس** (`?raw=1`)
جنب جوجل درايف. تنبيه للمستخدم إن الصورة بتفضل على السيرفر التاني.
**ملاحظة:** الموقع بيبعت `Cross-Origin-Resource-Policy: same-origin` (في `firebase.json`)،
فصور الموقع نفسه مش هتتحمّل في صفحة على نطاق تاني — ده بيأثر على الاختبار المحلي بس.

## منيو المخبز الكامل (2026-09-15)
171 منتج في 17 قسم اتضافوا من `Downloads/المنيو/المنيو` (مجلد لكل قسم، اسم الملف = اسم المنتج).
- الصور **ملفات ثابتة في الاستضافة**: `assets/menu/<slug>-NN.webp` (مش عقدة assets ولا درايف).
  546 ميجا PNG → **16 ميجا WebP** (ffmpeg، حد 900px، جودة 82). عقدة `menu` فضلت 40 ك.ب.
  السبب: `assets/**` عليها `immutable` لسنة، وبتتنشر مع الموقع، وبلا تضخّم base64 ولا كوتة RTDB.
- الأسماء الإنجليزية مترجمة آلياً (نفس خدمة `translate.js`) + تصحيح يدوي لأسماء الأصناف المعروفة.
- **كل الأسعار = 0** بطلب المالك؛ بتتعدّل من لوحة التحكم → المنيو.

## تحذير نشر: `seo/publish.js` بينشر الاستضافة **بس**
